...
router.post("/match", (req: Request, res: Response) => {
  const { deficitHospitalId, resourceId } = req.body as {
    deficitHospitalId?: string;
    resourceId?: string;
  };
  ...
  let shortestDistance = euclideanDistance(
    deficitHospital.location.latitude,
    deficitHospital.location.longitude,
    closestMatch.location.latitude,
    closestMatch.location.longitude
  );
...
router.post("/transfer", async (req: Request, res: Response) => {
  const { originHospitalId, destHospitalId, resourceId, quantity } =
    req.body as {
  ...
  if (
    ...
    res.status(400).json({
      error:
        "originHospitalId, destHospitalId, resourceId, and quantity are required",
    });
    return;
  }
  ...
  if (testReceiver) {
    ...
    console.log(
      "[Beacon] TEST_RECEIVER_NUMBER not set — transfer completed without SMS"
    );
  }
